...
class storeSystem(models.Model):
    ...
    endpointStore=models.ForeignKey(endpointSystem, on_delete=models.SET_NULL, null=True, blank=True)
...
class productSystem(models.Model):
    ...
    endpointProduct=models.ForeignKey(endpointSystem, on_delete=models.SET_NULL, null=True, blank=True)
    ...
    def getStockTotal(self):
        ...
        if len(stockProducts) == 0:
            ...
            for stock in stockProducts:
                totalStock = Decimal(totalStock) + Decimal(stock.quatityProduct)
            totalStock = str(Decimal('%.2f' % (Decimal(totalStock))))
            return totalStock

class storexproductSystem(models.Model):
    quatityProduct = models.CharField(max_length=16,null=True,blank=True)
    asociatedProduct = models.ForeignKey(productSystem, on_delete=models.SET_NULL, null=True, blank=True)
    asociatedStore = models.ForeignKey(storeSystem, on_delete=models.SET_NULL, null=True, blank=True)